...
pub fn main() !void {
    ...
    const fragment_path = "src/shaders/1_4_2_shader.fs";
    ...
    // Load texture
    var container_texture: c_uint = undefined;
    var emoji_texture: c_uint = undefined;
    ...
    defer zstbi.deinit();
    // Container texture
    // Filp the image on load
    zstbi.setFlipVerticallyOnLoad(true);
    const container_texture_info = zstbi.Image.info("textures/container.jpg");
    var container_texture_image = try zstbi.Image.loadFromFile(
        "textures/container.jpg",
        container_texture_info.num_components,
    );
    defer container_texture_image.deinit();

    gl.texImage2D(
        gl.TEXTURE_2D,
        0,
        gl.RGB,
        @intCast(container_texture_image.width),
        @intCast(container_texture_image.height),
        0,
        gl.RGB,
        gl.UNSIGNED_BYTE,
        container_texture_image.data.ptr,
    );
    // Emoji texture
    ...
    const emoji_texture_info = zstbi.Image.info("textures/awesomeface.png");
    var emoji_texture_image = try zstbi.Image.loadFromFile(
        "textures/awesomeface.png",
        emoji_texture_info.num_components,
    );
    defer emoji_texture_image.deinit();
    gl.texImage2D(
        gl.TEXTURE_2D,
        0,
        gl.RGB,
        @intCast(emoji_texture_image.width),
        @intCast(emoji_texture_image.height),
        0,
        gl.RGBA,
        gl.UNSIGNED_BYTE,
        emoji_texture_image.data.ptr,
    );
